...
class ResolveFtlTests(unittest.TestCase):
    ...
    def test_pruned_unreachable_pegging_returns_resolution_error(self) -> None:
        ...
        self.assertEqual(payload["error_code"], "PEGGING_NOT_FOUND")
        self.assertIn("해석 불가", payload["error"])

    def test_fetch_reports_missing_origin_without_exposing_remote(self) -> None:
        proc, payload = self.fixture.run("--fetch", "deadbeef")
        self.assertEqual(proc.returncode, 2)
        self.assertEqual(payload["error_code"], "PEGGING_NOT_FOUND")
        self.assertEqual(payload["fetch"], {
            "requested": True, "attempted": False, "status": "origin_missing",
        })
        self.assertNotIn("url", json.dumps(payload).lower())

    def test_fetch_reports_remote_failure_as_structured_status(self) -> None:
        git(self.fixture.integration, "remote", "add", "origin", str(self.fixture.integration))
        proc, payload = self.fixture.run("--fetch", "deadbeef")
        self.assertEqual(proc.returncode, 2)
        self.assertEqual(payload["error_code"], "PEGGING_NOT_FOUND")
        self.assertEqual(payload["fetch"], {
            "requested": True, "attempted": True, "status": "fetch_failed",
        })
        self.assertNotIn(str(self.fixture.integration), json.dumps(payload))

    def test_fetch_recovers_pegging_from_origin(self) -> None:
        remote = Path(self.temp.name) / "remote-integration"
        subprocess.run(
            ["git", "clone", "-q", str(self.fixture.integration), str(remote)], check=True
        )
        git(remote, "config", "user.name", "Test")
        git(remote, "config", "user.email", "test@example.com")
        git(remote, "commit", "-q", "--allow-empty", "-m", "remote pegging")
        remote_sha = git(remote, "rev-parse", "HEAD")
        git(self.fixture.integration, "remote", "add", "origin", str(remote))

        proc, payload = self.fixture.run("--fetch", remote_sha)
        self.assertEqual(proc.returncode, 0, proc.stderr)
        self.assertEqual(payload["pegging"]["sha"], remote_sha)

    def test_missing_ftl_object_has_specific_error_code(self) -> None:
        missing = "1" * 40
        p3 = self.fixture.pegging(missing, "missing FTL object")
        proc, payload = self.fixture.run(
            "--fetch", f"{self.fixture.p2}..{p3}"
        )
        self.assertEqual(proc.returncode, 2)
        self.assertEqual(payload["error_code"], "FTL_COMMIT_NOT_FOUND")
        self.assertEqual(payload["fetch"]["status"], "origin_missing")
    ...
    def test_uninitialized_submodule_returns_repo_error(self) -> None:
        ...
        self.assertFalse(payload["ok"])
        self.assertEqual(payload["error_code"], "FTL_REPO_INVALID")
        self.assertIn("submodule", payload["error"])

    def test_invalid_integration_repo_has_specific_error_code(self) -> None:
        not_repo = Path(self.temp.name) / "not-repo"
        not_repo.mkdir()
        proc = subprocess.run(
            [sys.executable, str(SCRIPT), "--repo", str(not_repo), "HEAD"],
            capture_output=True, text=True,
        )
        payload = json.loads(proc.stdout)
        self.assertEqual(proc.returncode, 3)
        self.assertEqual(payload["error_code"], "INTEGRATION_REPO_INVALID")
